...
def build(controller,config,verifier=None):
    ...
    def pcc_submit(project:str,grant_version:int,goal:str,plan:dict,request_label:str='')->dict:
        """Queue ONE Plus task. May write approved files, quarantine deletions, install pinned wheels or approved DSH/npm packages and transfer with approved targets. Plan: read_files, expected_outputs; publish [{artifact,destination}]; delete [relative file]; dependencies [approved name]; uploads [{artifact,target}]; optional downloads [{target,destination}] into frozen task input; optional package_installs [saved install ID] executed ONCE by broker after Plus results. No arbitrary package names, versions, commands or lifecycle scripts. Installation is not activation; inspect broker result. Read pcc_project first. No caller-supplied URL, credentials or permission overrides. Identical goal/plan/label returns existing task; new label only for user-explicitly distinct work. Platform confirmations remain required."""
